    ...
    try {
      ...
      if ($postData != 'saveResponse') {
        ...
        if (!isset($revProductId)) {
          ...
          Mage::helper('ReverbSync/data') -> reverbReports($productId, $product -> getName(), $product -> getSku(), $stock -> getQty(), null, 1, null);
        }
      }
    } catch (Exception $e) {
      Mage::log("test",1,"dd.log");
      ...
      if ($postData != 'saveResponse') {
        ...
        foreach ($order->getAllItems() as $item) {
          ...
          if ($revProductId && $revSync) {
            ...
            try {
              $responseId = Mage::helper('ReverbSync/data') -> updateObject($revConnection, $fieldsArray, 'listings', $revProductId);
              Mage::helper('ReverbSync/data') -> reverbReports($item -> getProductId(), $product -> getName(), $product -> getSku(), $prodQty, null, 1, null);
            } catch (Exception $e) {
              Mage::helper('ReverbSync/data') -> reverbReports($item -> getProductId(), $product -> getName(), $product -> getSku(), $prodQty, null, 0, $e -> getMessage());
            }
          }
        }
